...
class GeneralController extends Controller
{
    ...
    public function save_general_settings(Request $request)
    {
        ...
        if ($request->hasFile('logo_dark')) {
            ...
            $request->validate([
                'logo_dark' => 'image|mimes:jpeg,png,jpg|max:1024',
            ]);
            $file = $request->file('logo_dark');
            $extension = $file->getClientOriginalExtension();
            $filename = getRandomString().'-'.time() . '.' . $extension;
            $file->move('uploads/logo_dark', $filename);
            setSettings('logo_dark', 'uploads/logo_dark/'.$filename);
        }
        ...
        if ($request->developers) {
            setSettings('developers', request('developers'));
        }
        if ($request->developers_url) {
            setSettings('developers_url', request('developers_url'));
        }
